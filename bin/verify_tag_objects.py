...
##########################################
def generate_tag_list(ProcTag,ReqNumConstraint,dbh,dbSchema,verbose=0):
...
#
#       An alternative version of the above query that constrains directly to a partition
#       While this is roughly 10% faster, this may not give a complete answer if the observations
#       were not part of a standard temporary table load directly as a partition.
#
#        query="""SELECT 
#                c.filename as filename,
#                c.objects  as num_objects,
#                count(o.flux_auto) as num_se_objects 
#            FROM {schema:s}catalog c, {schema:s}se_object PARTITION (DESSE_REQNUM{rnum:07d}) o, gtt_id gi
#            WHERE c.pfw_attempt_id=gi.id
#                and c.filetype='cat_finalcut'
#                and c.filename=o.filename 
#                having count(o.flux_auto)!=c.objects 
...
#
    ...
    if (check_tag_exists(args.proctag,dbh,dbSchema,verbose)):
        if (verbose>0):
...
#
    t0=time.time()
    AttemptDict=generate_tag_list(args.proctag,ReqNumConstraint,dbh,dbSchema,verbose)
    t1=time.time()
    if (verbose > 0):
...
#        ProblematicDict=check_num_objects(AttemptDict,dbh,dbSchema,verbose)
        ProblematicDict=check_num_objects_v2(AttemptDict,dbh,dbSchema,verbose)
        t1=time.time()
        if (verbose > 0):
